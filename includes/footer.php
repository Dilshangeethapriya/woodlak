<?php 
$base_url = "http://localhost/woodlak"; 
?>

<footer class="bg-[#543310] text-white mt-10">
    <div class="mx-auto py-6 px-4 flex flex-col md:flex-row justify-between items-center">
        
        <div class="flex justify-center md:justify-start mb-4 md:mb-0">
            <a href="https://web.facebook.com/woodlak123" class="text-white mx-2 hover:text-gray-400">
                <i class="bi bi-facebook text-xl"></i>
            </a>
            <a href="https://wa.me/c/94753793553" class="text-white mx-2 hover:text-gray-400">
                <i class="bi bi-whatsapp text-xl"></i>
            </a>
            <a href="https://www.tiktok.com/@woodlak" class="text-white mx-2 hover:text-gray-400">
                <i class="bi bi-tiktok text-xl"></i>
            </a>

        </div>
         <div class="text-center md:text-left">
            <p>&copy; 2024 WOODLAK. All rights reserved. 
                <a href="<?php echo $base_url; ?>/orders/terms_and_conditions/termsAndCondition.php" class="text-white hover:text-gray-400 ml-2">Terms and Conditions</a>
            </p>
        </div>

       
        <div class="mt-4 md:mt-0">
            <a href="#" class="text-white hover:text-gray-400">
                <i class="bi bi-arrow-up-circle-fill text-2xl"></i>
            </a>
        </div>
    </div>
</footer>