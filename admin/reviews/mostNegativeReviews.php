...
// Fetch most negative reviews with pagination
if ($productID) {
    // Fetch most negative reviews for a specific product
    $mostNegativeQuery = $conn->prepare("
        ...
        LIMIT ?, ?
    ");
    $mostNegativeQuery->bind_param('iii', $productID, $offset, $reviewsPerPage);
} else {
    // Fetch most negative reviews across all products
    $mostNegativeQuery = $conn->prepare("
        SELECT rs.*, r.reviewID, r.reviewText, r.customerName, r.rating, r.createdAt 
        FROM review_sentiment rs
        JOIN review r ON rs.reviewID = r.reviewID
        ...
        LIMIT ?, ?
    ");
    $mostNegativeQuery->bind_param('ii', $offset, $reviewsPerPage);
}

$mostNegativeQuery->execute();
$mostNegativeReviews = $mostNegativeQuery->get_result();

// Get total number of most negative reviews for pagination
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_reply'])) {
    ...
    if ($stmt->execute()) {
        header("Location: mostNegativeReviews.php?page=$currentPage&productID=$productID");
        exit();
    } else {
...
<head>
    ...
    <title>Most Negative Reviews</title>
...
<div class="review-section mt-10 mb-36 max-w-screen-lg mx-auto rounded-md p-5" style="background-color: rgba(255, 255, 255, 0.8);">
    <h2 class="text-[#78350f] text-4xl mb-6 text-center">Most Negative Reviews</h2>

    <div id="reviewsSection">
        <?php if ($mostNegativeReviews->num_rows > 0): ?>
            <?php while ($review = $mostNegativeReviews->fetch_assoc()): ?>
            ...
            <?php endwhile; ?>
        <?php else: ?>
            <p class="text-yellow-800 flex items-center gap-2"><i class="bi bi-exclamation-circle-fill"></i> No negative reviews found.</p>
        <?php endif; ?>
    </div>
    ...
    <?php if ($totalPages > 1): ?>
        <div class="pagination mt-4 flex flex-wrap justify-center">
            <?php if ($currentPage > 1): ?>
                <button onclick="window.location.href='mostNegativeReviews.php?page=<?php echo ($currentPage - 1); ?>&productID=<?php echo $productID; ?>'" class="bg-[#9b734b] hover:bg-[#785937] text-white px-3 py-1 md:px-4 md:py-2 rounded-md m-1 text-sm md:text-base">Previous</button>
            <?php endif; ?>

            <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                <button onclick="window.location.href='mostNegativeReviews.php?page=<?php echo $i; ?>&productID=<?php echo $productID; ?>'" class="bg-<?php echo $i == $currentPage ? '[#9b734b]' : '[#C4A484]'; ?> text-<?php echo $i == $currentPage ? 'white' : 'black'; ?> px-3 py-1 md:px-4 md:py-2 rounded-md m-1 text-sm md:text-base"><?php echo $i; ?></button>
            <?php endfor; ?>

            <?php if ($currentPage < $totalPages): ?>
                <button onclick="window.location.href='mostNegativeReviews.php?page=<?php echo ($currentPage + 1); ?>&productID=<?php echo $productID; ?>'" class="bg-[#9b734b] hover:bg-[#785937] text-white px-3 py-1 md:px-4 md:py-2 rounded-md m-1 text-sm md:text-base">Next</button>
            <?php endif; ?>
        </div>
    <?php endif; ?>
