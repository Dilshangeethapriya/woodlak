...
(function ($, window) {
  ...
  api.update = function (frame) {
    return this.each(function () {
      ...
      if (frame === undefined) {
        data.frame +=
          data.animation && data.reverse ? -data.frameStep : data.frameStep;
      } else {
        data.frame = frame;
      }
      ...
      if (
        data.animation || // wrap frame during animation
        (!data.animation && data.frameWrap)
      ) {
        // wrap frame during user input
